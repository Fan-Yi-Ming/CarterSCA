...
if __name__ == '__main__':
    # ============================ USIM目标设备初始化 ============================
    target_uz_usim = TargetUsim(port="COM1", baudrate=115200, timeout=2.0, card_type=1)
    target_uz_usim.init()
    ...
    # 固定组数据采集次数
    gatherer_sds804x_tvla_acquisition_fix_times = 2000
    # 随机组数据采集次数
    gatherer_sds804x_tvla_acquisition_rnd_times = 2000
    # TraceSet数据文件保存路径
    gatherer_sds804x_traceset_path = "..\\..\\traceset\\tvla_test.trs"
    ...
    # 定义TVLA_GROUP参数：1字节组别标识，BYTE类型，参数块偏移量32
    # 组别编码：0x00=固定组(FIXED)，0x01=随机组(RANDOM)
    trace_parameter_definition_map["TVLA_GROUP"] = TraceParameterDefinition(ParameterType.BYTE, 1, 32)
    ...
    for i in range(gatherer_sds804x_tvla_acquisition_fix_times):
        ...
        # 生成固定认证参数（用于固定组测试）
        rnd_hex = "FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF"  # 16字节固定随机数
        ...
        trace_parameter_map["RND"] = tp.ByteArrayParameter(rnd)  # 设置RND参数值
        trace_parameter_map["ATUN"] = tp.ByteArrayParameter(atun)  # 设置ATUN参数值
        trace_parameter_map["TVLA_GROUP"] = tp.ByteArrayParameter(bytes([0x00]))  # 设置组别为固定组(0x00)

        # 执行USIM设备认证流程（触发目标设备运行）
        target_uz_usim.process(rnd_hex, autn_hex)
        ...
        print(f"固定组第 {i + 1} 次采集耗时: {elapsed_time:.2f} 秒，共 {gatherer_sds804x_tvla_acquisition_fix_times} 次")
    ...
    for i in range(gatherer_sds804x_tvla_acquisition_rnd_times):
        ...
        # 生成随机认证参数（用于随机组测试）
        rnd_hex = generate_random_hex_string(16)  # 生成16字节真随机数
        autn_hex = "FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF"  # 16字节固定认证令牌
        rnd = bytes.fromhex(rnd_hex)
        atun = bytes.fromhex(autn_hex)
        ...
        trace_parameter_map["RND"] = tp.ByteArrayParameter(rnd)  # 设置RND参数值
        trace_parameter_map["ATUN"] = tp.ByteArrayParameter(atun)  # 设置ATUN参数值
        trace_parameter_map["TVLA_GROUP"] = tp.ByteArrayParameter(bytes([0x01]))  # 设置组别为随机组(0x01)
        ...
        end_time = time.monotonic()
        elapsed_time = end_time - start_time
        print(f"随机组第 {i + 1} 次采集耗时: {elapsed_time:.2f} 秒，共 {gatherer_sds804x_tvla_acquisition_rnd_times} 次")
