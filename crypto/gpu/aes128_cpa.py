...
class Aes128CPA:
    ...
    def analyze(self):
        ...
        start_time = time.perf_counter()
        for i in self.sbox_index_arr:
            correlation_arr_2d = analyze_process_cpa_gpu(self.data_arr_3d[i], self.sample_arr_2d,
                                                         self.batch_size)
            (self.sbox_key_arr_2d[i],
             self.sbox_keyvalue_arr_2d[i],
...
if __name__ == '__main__':
    ...
    # 第一轮攻击配置（加密）
    aes128_cpa.traceset_path = "D:\\traceset\\c51_aes128\\aes128_en+LowPass(203439)+StaticAlign(203755).trs"
    aes128_cpa.traceset2_switch = False
    aes128_cpa.sample_first_pos = 450000
    aes128_cpa.sample_number = 30000
    aes128_cpa.crypto_direction = 0
    aes128_cpa.sbox_index_arr = index_str_to_range("0-15")
    aes128_cpa.analyze()

    # 第二轮攻击配置（解密）
    aes128_cpa.traceset_path = "D:\\traceset\\c51_aes128\\aes128_de+LowPass(211850)+StaticAlign(212310).trs"
    aes128_cpa.traceset2_switch = False
    aes128_cpa.sample_first_pos = 470000
    aes128_cpa.sample_number = 30000
